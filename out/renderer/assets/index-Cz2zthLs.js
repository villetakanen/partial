...
const eager_effects = /* @__PURE__ */ new Set()
  ...
  return new Proxy(
    ...
    value,
    {
      ...
      has(target2, prop) {
        ...
        if (
          s !== void 0 ||
          (active_effect !== null && (!has || get_descriptor(target2, prop)?.writable))
        ) {
          if (s === void 0) {
            s = with_parent(() => {
...
function handle_event_propagation(event) {
  ...
  if (handled_at) {
    ...
    if (at_idx !== -1 && (this === document || this === /** @type {any} */ window)) {
      event.__root = this
      return
    }
  ...
  try {
    ...
    if (throw_error) {
      for (const error of other_errors) {
        queueMicrotask(() => {
          throw error
        })
      }
...
function from_html(content, flags2) {
  ...
  return () => {
    if (node === void 0) {
      ...
      node = /** @type {TemplateNode} */ /* @__PURE__ */ get_first_child(node)
    }
    var clone =
      /** @type {TemplateNode} */
...
const mounted_components = /* @__PURE__ */ new WeakMap()
class BranchManager {
  /** @type {TemplateNode} */
  anchor
  /** @type {Map<Batch, Key>} */
...
function require_baseFilter() {
  ...
  function baseFilter(collection, predicate) {
    var result = []
    baseEach(collection, (value, index, collection2) => {
...
function require_baseMatches() {
  ...
  function baseMatches(source2) {
    ...
    return (object) => object === source2 || baseIsMatch(object, source2, matchData)
  }
...
function require_baseMap() {
  ...
  function baseMap(collection, iteratee) {
    ...
    baseEach(collection, (value, key, collection2) => {
...
function requireTransform() {
  ...
  function transform(object, iteratee, accumulator) {
    var isArr = isArray(object),
      isArrLike = isArr || isBuffer2(object) || isTypedArray(object)
    ...
    ;(isArrLike ? arrayEach : baseForOwn)(object, (value, index, object2) =>
      iteratee(accumulator, value, index, object2),
    )
    return accumulator
  }
...
function requireUnion() {
  ...
  var union = baseRest((arrays) => baseUniq(baseFlatten(arrays, 1, isArrayLikeObject, true)))
  union_1 = union
  return union_1
}
var _baseValues
...
function requireGraph() {
  ...
  Graph2.prototype.removeNode = function (v) {
    if (_.has(this._nodes, v)) {
      ...
      if (this._isCompound) {
        ...
        _.each(this.children(v), (child2) => {
          this.setParent(child2)
        })
  ...
  function edgeArgsToId(isDirected, v_, w_, name) {
    ...
    return (
      v + EDGE_KEY_DELIM + w + EDGE_KEY_DELIM + (_.isUndefined(name) ? DEFAULT_EDGE_NAME : name)
    )
  }
...
function requireJson() {
  ...
  function read(json2) {
    ...
    _.each(json2.nodes, (entry) => {
      g.setNode(entry.v, entry.value)
      if (entry.parent) {
        g.setParent(entry.v, entry.parent)
      }
    })
    _.each(json2.edges, (entry) => {
...
function requireFindCycles() {
  ...
  function findCycles(g) {
    return _.filter(
      tarjan(g),
      (cmpt) => cmpt.length > 1 || (cmpt.length === 1 && g.hasEdge(cmpt[0], cmpt[0])),
    )
  }
  return findCycles_1
}
...
var root$3 = /* @__PURE__ */ from_html(
  `<section class="gantt-view svelte-80wpic"><p class="placeholder svelte-80wpic"> </p></section>`,
)
function Gantt($$anchor, $$props) {
  push($$props, true)
  const taskCount = /* @__PURE__ */ user_derived(() => $$props.plan.tasks.length)
  const edgeCount = /* @__PURE__ */ user_derived(() => $$props.dag.edgeCount())
  var section = root$3()
  var p = child(section)
  var text = child(p)
  template_effect(() =>
    set_text(
      text,
      `Gantt view — ${get(taskCount) ?? ''} tasks, ${get(edgeCount) ?? ''} dependencies`,
    ),
  )
...
var root$2 = /* @__PURE__ */ from_html(
  `<section class="graph-view svelte-17lrwva"><p class="placeholder svelte-17lrwva"> </p></section>`,
)
function Graph($$anchor, $$props) {
  ...
  template_effect(() =>
    set_text(
      text,
      `Graph view — ${get(taskCount) ?? ''} tasks, ${get(edgeCount) ?? ''} dependencies`,
    ),
  )
  append($$anchor, section)
  pop()
...
var root$1 = /* @__PURE__ */ from_html(
  `<section class="kanban-view svelte-kubzvj"><p class="placeholder svelte-kubzvj"> </p></section>`,
)
function Kanban($$anchor, $$props) {
  ...
  template_effect(() =>
    set_text(
      text,
      `Kanban view — ${get(taskCount) ?? ''} tasks, ${get(edgeCount) ?? ''} dependencies`,
    ),
  )
  append($$anchor, section)
  pop()
...
var root = /* @__PURE__ */ from_html(
  `<main class="svelte-1la1gos"><header class="svelte-1la1gos"><h1 class="svelte-1la1gos">Partial</h1> <nav class="svelte-1la1gos"><button>Gantt</button> <button>Kanban</button> <button>Graph</button></nav></header> <section class="view-container svelte-1la1gos"><!></section></main>`,
)
function App($$anchor, $$props) {
  push($$props, true)
  const view = /* @__PURE__ */ state('gantt')
  const emptyPlan = { version: '1.0.0', project: '', tasks: [] }
  const plan = /* @__PURE__ */ state(proxy(emptyPlan))
  const dag = /* @__PURE__ */ user_derived(() => buildDAG(get(plan).tasks))
  const api = window.api
  user_effect(() => {
    api?.onPlanUpdated((payload) => {
      set(plan, payload.plan, true)
    })
  })
  var main = root()
  ...
  button.__click = () => set(view, 'gantt')
  let classes
  var button_1 = sibling(button, 2)
  button_1.__click = () => set(view, 'kanban')
  let classes_1
  var button_2 = sibling(button_1, 2)
  button_2.__click = () => set(view, 'graph')
  let classes_2
  ...
  var alternate_1 = ($$anchor2) => {
    ...
    if_block(
      node_1,
      ($$render) => {
        if (get(view) === 'kanban') $$render(consequent_1)
  ...
  if_block(node, ($$render) => {
    if (get(view) === 'gantt') $$render(consequent)
    else $$render(alternate_1, false)
  })
  template_effect(() => {
    classes = set_class(button, 1, 'svelte-1la1gos', null, classes, {
      active: get(view) === 'gantt',
    })
    classes_1 = set_class(button_1, 1, 'svelte-1la1gos', null, classes_1, {
      active: get(view) === 'kanban',
    })
    classes_2 = set_class(button_2, 1, 'svelte-1la1gos', null, classes_2, {
      active: get(view) === 'graph',
    })
  })
  append($$anchor, main)
  pop()
